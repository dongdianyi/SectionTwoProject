...
//		if (this.examineeId != null && !"".equals(this.examineeId)) {
        ...
        int touch = touch(angle, gps, this.place, this.car, 0.0, 0.0);// ture: 越出边界; false:未越出边界
//		if (touch) {// 越出边界
        return touch;
//		} else {
//			return/ touch;
...
//				}
     ...
     * @return ture: 越出边界; false:未越出边界
     */
    public int touch(double angle, Point gps, List<Point> place, List<Point> car, Double outLine, Double pileLine) {
        if (outLine == null) {
            outLine = 0.0;
        }
        if (pileLine == null) {
        ...
        if (isOut) {// 出界
            boolean isPile = pileTouches(carGeo, place, 3 + pileLine);// 碰到触发线
            if (isPile) {// 撞杆
                return PILE_ERROR;
            }
            return OUT_ERROR;
